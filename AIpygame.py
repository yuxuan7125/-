...
pygame.display.set_caption("Pool Cards Game")
...
def game_screen(player_count, safe_count):
    global pool_cards, players, total_money, forced_draws, current_bet
    ...
    players[:] = [Player(i, (0,0)) for i in range(player_count)]
    setup_game()
    ...
    while True:
        ...
        # 畫玩家
        for p in players:
            pygame.draw.circle(screen, (255,255,255), p.pos, 40, 2)
            id_text = FONT.render(f"P{p.idx+1}", True, (200,200,200))
            screen.blit(id_text, (p.pos[0] - id_text.get_width()//2, p.pos[1] - 25))
            money_text = FONT.render(str(p.money), True, p.color)
